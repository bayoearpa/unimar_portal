...
function get_data_join_makul_us_temp ($where){
	$this->db->select('tmst_mata_kuliah.Nama_mata_kuliah as nama_makul,
		tmst_mata_kuliah.SKS_mata_kuliah as sks,
		tbl_kliring_us_makul_temp.id_smta_makul as id_makul_temp,
		tmst_mata_kuliah.Kode_mata_kuliah as kd_makul');
		$this->db->from('tbl_kliring_us_makul_temp');
		$this->db->join('tmst_mata_kuliah',' tbl_kliring_us_makul_temp.Kode_mata_kuliah = tmst_mata_kuliah.Kode_mata_kuliah','inner');
		$this->db->where($where);
		//$this->db->order_by('tbl_catar_validasi.no_reg', "asc");
		$query=$this->db->get();
		return $query;
}
function get_data_join_makul_us ($where){
	$this->db->select('tmst_mata_kuliah.Nama_mata_kuliah as nama_makul,
		tmst_mata_kuliah.SKS_mata_kuliah as sks,
		tbl_kliring_us_makul.id_smta_makul as id_makul_temp,
		tmst_mata_kuliah.Kode_mata_kuliah as kd_makul');
...
function get_data_formon_mhsall($limit, $offset)
{
	# code...
	 // Mengambil data mahasiswa dengan tahun_masuk dari tahun 2018
	    ...
	    $this->db->where('tmst_mahasiswa.Tahun_masuk >=', '2021');
...
function get_data_formon_mhsyear_sb($year)
{
	# code...
	 // Mengambil data mahasiswa dengan tahun_masuk sesuai dengan yang dipilih
        ...
        $this->db->join('tmst_program_studi','tmst_mahasiswa.Kode_program_studi = tmst_program_studi.Kode_program_studi','inner');
        $this->db->join('tbl_mon','tmst_mahasiswa.NIM = tbl_mon.nim','inner');
        $this->db->where('tmst_mahasiswa.Tahun_masuk', $year);
         $this->db->where('tbl_mon.pra_status >=', 'sudah');
         $this->db->where('tbl_mon.status_sb >=', 'iya');
        $this->db->where_in('tmst_mahasiswa.Kode_program_studi', array('92403', '92402'));
        $query = $this->db->get();
...
function get_data_mhs_detail($id)
    {
        // Gantilah 'nama_tabel' dengan nama tabel yang sesuai dalam database Anda
	     $this->db->select('tmst_mahasiswa.NIM as nim,
	        ...
	        tbl_mon.upload_file_konprala as upload_file_konprala,
	        tbl_mon.nama_perusahaan,
    ...
    function get_data_mhs_detail2($id)
    {
        // Gantilah 'nama_tabel' dengan nama tabel yang sesuai dalam database Anda
	     $this->db->select('tmst_mahasiswa.NIM as nim,
			tmst_mahasiswa.Nama_mahasiswa as nama,
...
//////////////////////---------------- end off
////////////---------- untuk trb
function get_data_formon_mhsall_mod($limit, $offset)
{
	# code...
	 // Mengambil data mahasiswa dengan tahun_masuk dari tahun 2018
        $this->db->select('tmst_mahasiswa.NIM as nim,
	    ...
	    $this->db->join('tbl_mon','tmst_mahasiswa.NIM = tbl_mon.nim','inner');
	    $this->db->where('Tahun_masuk >=', '2021');
	    $this->db->where('tbl_mon.status_modeling =', 'sudah');
	    $this->db->where_in('tmst_mahasiswa.Kode_program_studi', array('92403', '92402'));
	    // $this->db->limit($limit, $offset); // Apply pagination
	    $query = $this->db->get();
	    return $query->result();
}
function count_all_data_formon_mhsall_mod()
{
    $this->db->select('COUNT(*) as count');
    $this->db->from('tmst_mahasiswa');
    $this->db->where('Tahun_masuk >=', '2021');
...
function get_data_formon_mhsyear_mod($year)
{
	# code...
	 // Mengambil data mahasiswa dengan tahun_masuk sesuai dengan yang dipilih
        $this->db->select('tmst_mahasiswa.NIM as nim,
...
function get_data_formon_mhsprodi_mod($program_studi)
{
	# code...
	 // Mengambil data mahasiswa dengan tahun_masuk sesuai dengan yang dipilih
        $this->db->select('tmst_mahasiswa.NIM as nim,
...
function get_data_formon_mhsyearnprodi_mod($year, $program_studi)
{
	# code...
	 // Mengambil data mahasiswa dengan tahun_masuk sesuai dengan yang dipilih
        $this->db->select('tmst_mahasiswa.NIM as nim,
...
function get_data_formon_mhs($id)
    ...
    function get_data_join_tkbi($where){
		$this->db->select('diklat_tkbi_peserta.id_tkbi,
			...
			diklat_tkbi_peserta.no_wa as no_wa,
			...
			diklat_tkbi_pembayaran.id_tkbi_bayar,
			diklat_tkbi_pembayaran.id_tkbi,
			diklat_tkbi_pembayaran.bayar,
