 <script>
    $(document).ready(function() {
        $('#filter-form').submit(function(e) {
            ...
            $.ajax({
                type: 'GET',
                url: '<?php echo base_url('ppk/mon_llspradata'); ?>',
...
// Menampilkan modal saat tombol "Edit" diklik
  $('#example31082023').on('click', '.edit-button', function() {
    ...
    // Ambil data yang akan diedit dari server dengan AJAX
    $.ajax({
      url: '<?php echo base_url('ppk/mon_praedit/'); ?>' + id, // Sesuaikan dengan URL yang sesuai
      type: 'GET',
      success: function(data) {
        // Isi modal dengan data yang diambil
        console.log(data); // Cetak nilai data ke konsol
...
function connectEditButtonListeners() { 
  ...
  $('.edit-button').click(function() {
    ...
    $('#saveEdit').click(function() {
        $.ajax({
            url: '<?php echo base_url('ppk/mon_praeditp'); ?>', // Sesuaikan dengan URL yang sesuai
            type: 'POST',
